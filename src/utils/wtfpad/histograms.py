...
from utils.wtfpad.constants import INF
...
class Histogram:
    ...
    def __init__(self, hist: dict, interpolate: bool = True, remove_tokens: bool = False, decay_by: float = 0,
                 name: str = ''):
        """Initialize a histogram.

        `hist` is a dictionary. The keys are labels of a histogram. They represent
    ...
    @classmethod
    def get_intervals_from_endpoints(cls, ep_list):
    ...
    @classmethod
    def divide_histogram(cls, histogram, divide_by=None):
    ...
    @classmethod
    def skew_histo_one_bin(cls, d, side='left'):
    ...
    @classmethod
    def skew_histo(cls, d, nbins, side="left"):
    ...
    @classmethod
    def get_dict_histo_from_list(cls, l):
        import numpy as np
        counts, bins = np.histogram(l, bins=cls.create_exponential_bins(a=0, b=10, n=20))
        d = dict(zip(list(bins) + [INF], [0] + list(counts) + [0]))
        d[0] = 0  # remove 0 iner-arrival times
        return d

    @classmethod
    def dict_from_list(cls, l, num_samples=1000):
        import numpy as np
        counts, bins = np.histogram([math.ceil(v) for v in random.sample(l, num_samples)],
                                    bins=cls.create_exponential_bins(a=0, b=10, n=20))
        d = dict(zip(list(bins) + [INF], [0] + list(counts) + [0]))
        d[0] = 0  # remove 0 iner-arrival times
        return d

    @classmethod
    def dict_from_distr(cls, name, params, scale=1.0, num_samples=10000, bin_size=50):
        import numpy as np

        if name == "weibull":
            shape = params
            counts, bins = np.histogram(np.random.weibull(shape, num_samples) * scale,
                                        bins=cls.create_exponential_bins(a=0, b=10, n=bin_size))
    ...
    @classmethod
    def create_exponential_bins(cls, sample=None, min_bin=None,
